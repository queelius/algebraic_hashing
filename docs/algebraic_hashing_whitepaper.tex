...
\subsection{Hash Function Composition}

The idea of composing hash functions has been explored in various contexts, though primarily in cryptography. Preneel et al.~\cite{composition} introduced systematic approaches to constructing hash functions from block ciphers, examining different composition modes and their security properties. Bellare and Micciancio~\cite{algebraic} proposed using algebraic structures for collision-resistant hashing, though their focus was on cryptographic security rather than general-purpose hashing.
...
Perfect hashing has a rich theoretical foundation. Carter and Wegman~\cite{universal} introduced universal hash families, which form the basis for many perfect hashing schemes. Their key insight was that randomly selecting a hash function from a universal family provides probabilistic guarantees on collision probability.

Fredman, Komlós, and Szemerédi~\cite{perfect} presented the FKS scheme, a two-level perfect hashing construction that achieves $O(n)$ space and $O(1)$ worst-case lookup time. Our implementation closely follows the FKS approach, adapting it to modern C++ with generic hash function support through templates.

More recent work on minimal perfect hash functions (MPHFs) includes CHD~\cite{chd}, BDZ~\cite{bdz}, and RecSplit~\cite{recsplit}, which achieve better space efficiency than FKS. However, these specialized algorithms sacrifice generality and composability. Our library prioritizes a uniform algebraic interface over space-optimality, making it suitable for integration with arbitrary hash functions.

\subsection{Non-Cryptographic Hash Functions}

The FNV hash~\cite{fnv} exemplifies simple, fast non-cryptographic hashing with good statistical properties. More recent developments include xxHash~\cite{xxhash} and CityHash, which achieve exceptional speed through careful algorithm design and SIMD utilization. MurmurHash and its successor, SMHasher, provide comprehensive test suites for hash quality.
...
\subsection{C++ Template Metaprogramming and Concepts}

The design of our library builds heavily on modern C++ techniques. Vandevoorde et al.~\cite{template} provide comprehensive coverage of C++ templates, including expression templates and compile-time computation. Sutton et al.~\cite{concepts} introduced concepts to C++20, enabling constraint-based template specialization and improved error messages.
...
\subsection{Algebraic Approaches in Cryptography}
...
\subsection{Algebraic Properties of Hash Functions}
...
Given hash functions $h_1, h_2 \in \mathcal{H}$ with the same domain and codomain, we define XOR composition:
$$
...
\begin{proposition}[Algebraic Properties of XOR Composition]
XOR composition of hash functions satisfies the following properties:
\begin{itemize}
    \item \textbf{Associativity}: $(h_1 \oplus h_2) \oplus h_3 = h_1 \oplus (h_2 \oplus h_3)$
    \item \textbf{Commutativity}: $h_1 \oplus h_2 = h_2 \oplus h_1$
    \item \textbf{Self-Inverse}: $h \oplus h$ yields the constant zero function
    \item \textbf{Closure}: $h_1 \oplus h_2 \in \mathcal{H}$ (composition produces a valid hash function)
\end{itemize}
\end{proposition}

\begin{proof}
These properties are inherited from the algebraic structure of $(\{0,1\}^n, \oplus)$, which forms an abelian group. For any $x \in D$:
\begin{itemize}
    \item Associativity: $(h_1 \oplus h_2 \oplus h_3)(x) = h_1(x) \oplus (h_2(x) \oplus h_3(x)) = (h_1(x) \oplus h_2(x)) \oplus h_3(x)$
    \item Commutativity: $(h_1 \oplus h_2)(x) = h_1(x) \oplus h_2(x) = h_2(x) \oplus h_1(x) = (h_2 \oplus h_1)(x)$
    \item Self-Inverse: $(h \oplus h)(x) = h(x) \oplus h(x) = 0^n$
\end{itemize}
The composed function $h_1 \oplus h_2$ maps $D \rightarrow C$, hence closure holds.
\end{proof}

\begin{remark}
While $\mathcal{H}$ does not form a group under XOR composition (there is no identity element among proper hash functions), the operation provides a structured way to combine hash functions while preserving desirable statistical properties, as we demonstrate empirically in Section 4.
\end{remark}
...
Our library implements a two-level perfect hashing scheme based on the FKS construction~\cite{perfect}, which relies on universal hashing~\cite{universal}:

\begin{theorem}[Two-Level Perfect Hashing~\cite{perfect}]
...
A hash function $h$ exhibits the strict avalanche criterion (SAC) if, when a single input bit is flipped, each output bit changes with probability approximately $\frac{1}{2}$.
\end{definition}

\begin{proposition}[Empirical Avalanche Preservation]
When $h_1$ and $h_2$ are distinct hash functions (e.g., using different seeds) that individually satisfy the avalanche criterion, XOR composition $h_1 \oplus h_2$ tends to preserve good avalanche properties, though typically with slightly degraded performance compared to the individual functions.
\end{proposition}

\begin{remark}
A rigorous proof of avalanche preservation would require independence assumptions about $h_1$ and $h_2$ that are difficult to justify for deterministic hash functions operating on the same input. Instead, we provide empirical validation in Section 4.3.
...
\begin{lstlisting}[caption={XOR hash function composition (simplified)}]
...
// Usage: compose FNV hashes with different seeds
xor_hash_fn_compose<fnv_hash, fnv_hash> composed{
    fnv_hash{0x12345678},
    fnv_hash{0x87654321}
};
\end{lstlisting}
...
The Fowler-Noll-Vo (FNV) hash~\cite{fnv} provides excellent distribution properties with minimal computational overhead. Our implementation supports seeded hashing to enable meaningful composition:

\begin{lstlisting}[caption={FNV-1a hash function with seed support}]
struct fnv_hash {
...
\textbf{FNV (ours)} & 209.2 & 22.1 & 4.4 & 1.1 \\
\textbf{FNV$\oplus$FNV} & 120.5 & 11.3 & 2.2 & 0.5 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Statistical quality metrics (ideal avalanche $\approx$ 0.5, ideal entropy = 1.0)}
\begin{tabular}{lrrr}
\toprule
\textbf{Hash Function} & \textbf{Avalanche} & \textbf{Entropy} & \textbf{std::hash baseline} \\
\midrule
FNV & 0.4805 & 1.0000 & --- \\
...
\bibitem{chd}
Belazzougui, D., Botelho, F. C., and Dietzfelbinger, M. (2009).
\textit{Hash, Displace, and Compress}.
Proceedings of the 17th Annual European Symposium on Algorithms (ESA), 557-568.

\bibitem{bdz}
Botelho, F. C., Pagh, R., and Ziviani, N. (2007).
\textit{Simple and Space-Efficient Minimal Perfect Hash Functions}.
Proceedings of the 10th International Workshop on Algorithms and Data Structures (WADS), 139-150.

\bibitem{recsplit}
Esposito, E., Pibiri, G. E., and Vigna, S. (2020).
\textit{RecSplit: Minimal Perfect Hashing via Recursive Splitting}.
Proceedings of the 22nd Workshop on Algorithm Engineering and Experiments (ALENEX), 175-185.
