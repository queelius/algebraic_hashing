...
An important operation that is closed over the free semigroup of $\{0,1\}$ is \emph{concatentation}, $# : \{0,1\}^* \times \{0,1\}^* \mapsto \{0,1\}^*$, which is defined as $a_1 \ldots a_n \cat b_1 \ldots b_m = a_1 \ldots a_n b_1 \ldots b_m$ with special cases $x # \epsilon = \epsilon # x = x$.
...
$\mathrm{pad} : \{0,1\}^* \times \mathbbm{Z}_{\geq 0} \mapsto \{0,1\}^*$ is defined as $\mathrm{pad}(x,k) = x # 0^{k-\mathrm{BL}(x)}$ with the special case $\mathrm{pad}(x,0) = \epsilon$.
...
We denote the mapping described by \cref{def:mapping} with the postfix function $' : \{0,1\}^* \mapsto \mathbbm{N}$ and $' : \mathbbm{N} \mapsto \{0,1\}^*$, which are inverse functions, i.e., $x'' = x$.
...
We search \emph{all} possible hash functions which are a function of $\mathrm{hash}$, an approximate random oracle, and choose the \emph{perfect hash function} on a specified set that has the \emph{smallest} bit length. We describe the algorithm for performing this exhaustive search in \cref{alg:ph}.


We consider a family of perfect hash functions for a set $\mathbbm{S}$ which are given by the output of a hash function $\mathrm{hash}$ that approximates a \emph{random oracle} applied to the input $x \in \mathbbm{S}$ concatenated with a bit string $b$. We describe the generative algorithm for the perfect hash function in \cref{alg:ph}. Note that in \cref{alg:ph}, the concatenation of two bit strings $x$ and $y$ is denoted by $x \cat y$.
...
\end{theorem}
\begin{proof}
First, in \cref{alg:ph} on Line~5, $b_n$ is a bit string such that each $x \in \mathbbm{S}$ concatenated with $b_n$ hashes to a unique integer in $\{1,\ldots,N\}$ by the hash function $\mathrm{hash}$, thus the hash function found perfectly hashes the elements of $\mathbbm{S}$. 

Second, by \cref{def:randomoracle}, $\mathrm{hash}$ approximates a random oracle whose output is uniformly distributed over the elements of $\{0,1\}^{n}$. Viewing the elements of $\{0,1\}^{n}$ as integers, where $|\{0,1\}^n| = 2^n$, $\mathrm{hash}$ is uniformly distributed over $\{0,1,\ldots,2^n-1\}$.
...
By the property of the hash function $\mathrm{hash}^*$ being a random oracle, the algorithm defined in \cref{dummy} randomly samples one of the hash functions, which is a perfect hash function with probabilty $p$.
...
\end{equation}
By \cref{def:mapping}, the $n$-th trial uniquely maps to a bit string of length $m = \lfloor \log_2 n \rfloor$. Thus, the expected bit length is given approximately by
\begin{align}
    \Expect{\log_2 Q}
...
The entropy of the random bit length $N$ is given by
\begin{equation}
?    
\end{equation}
\begin{proof}
\begin{align}
    -\sum_{n=0}^{\infty} \log_2\!\left(p_N(n | m,r)\right) p_{N}(n | m,r).
\end{align}
\end{proof}
...
\begin{algorithm}
	...
	\func{\mathrm{make\_hash}{$\mathbbm{X}$, $r$, $k$}}
	{
		...
		$\mathbbm{X}[\ell] = \{x \in \mathbbm{X}|\mathrm{trunc}\!\left(\mathrm{hash}(x' # 0'),t\right)' \mod k = \ell}$\;
		...
		\For{$\ell \in \{0,\ldots,k-1\}$}
		{
			\For{$n \gets 1$ \KwTo $\infty$}
			{
				...
				{
					$\beta = \mathrm{trunc}\!\left(\mathrm{hash}(x' # n'),N\right)$\;
...
\begin{algorithm}
	...
	\func{\makehash{$\mathbbm{X}$, $r$, $k$}}
	{
		...
		\For{$\ell \gets 1$ \KwTo $k$}
		{
			\For{$n \gets 1$ \KwTo $\infty$}
			{
				$\beta(x) = \mathrm{trunc}\!\left(\mathrm{hash}(x' # n'),N\right)$\;
...
\begin{equation}
	\Pr\bigl \{Q_t \leq q | \mathlarger{\cap}_{u=t+1}^{m} (Q_{u} > q) \bigr \}
\end{equation}



The \emph{data structure} for the rate-distorted perfect hash function with a maximum bit length $b$ is defined as $\PH(b) = \{0,1\}^{b} \times \mathbbm{N} \times \mathbbm{Q}$.
An optimal constructor for $\PH(b)$ with a load factor $r$,
\begin{equation}
	\mathrm{ph} : 2^{\mathbbm{U}} \times \mathbbm{Q} \times \mathbbm{N} \mapsto \mathrm{PH}(b),
\end{equation}
is given by
\begin{equation}
	\mathrm{ph}(\mathbbm{X}, r, b) = (n',N, \alpha)
\end{equation}
where
\begin{equation}
\begin{split}
\ell		& = 2^b, m = |\mathbbm{X}|, N = m / r, k = \lceil \log_2 N \rceil,\\
\beta(x,n) 	& = \mathrm{pad}\left(\mathrm{hash}(x' \cat n') \mod N, k\right),\\
\mathbbm{Y}[n] 	& = \{\beta(x,n) \in \{0,1\}^k | x \in \mathbbm{X}\},\\
\gamma(k)	& = \bigl \{j \in \mathbbm{N}_{\leq \ell} | \mathbbm{Y}[j] \in 2^{\{0,1\}^N} \land |\mathbbm{Y}[j]| = k \bigr \},\\
p			& = \argmax_j \{ j | \gamma(j) \neq \emptyset},\\
\alpha		& = 1-\frac{p}{m},\\
n			& = \min \gamma(p).
\end{split}
\end{equation}
...
%	$N = \frac{m}{r}$
%	\begin{equation}
%		K_j \sim \mathrm{GEO}!\bigl(\frac{N-1}{N}\bigr)
%	\end{equation}
%	$\Pr[K_j \leq k] = 1 - \left(1 - \frac{N-j+1}{N}\right)^{k+1}$
%	A maximum of $k$ hashes...
%	$p(k) = \Pr[K_1 \leq k \cap \cdots \cap K_m \leq k] = \prod_{j=1}^{m}\left(1 - \left(\frac{j-1}{N}\right)^{k+1}\right)$
%	$p(k) = \Pr[K_1 \leq k \cap \cdots \cap K_m \leq k] = \prod_{j=1}^{m-1}\left(1 - \left(\frac{j}{N}\right)^{k+1}\right)$
%	$p(k) = \Pr[K_1 \leq k \cap \cdots \cap K_m \leq k] = \prod_{j=1}^{m-1}\frac{N^{k+1} - j^{k+1}}{N^{k+1}}$
%	$p(k) = \Pr[K_1 \leq k \cap \cdots \cap K_m \leq k] = N^{-(m-1)(k+1)} \prod_{j=1}^{m-1}\left(N^{k+1} - j^{k+1}\right)$
%\end{proof}
...
%
%$\SetIndicator{\mathbbm{A}} : \mathbbm{X} \mapsto \{0,1\}$
%$\SetIndicator{\mathbbm{A}}(x) = \mathrm{trunc}(\mathrm{hash}(x),k) = \Fun{p}\Fun{?}[\mathbbm{A}]$

\section{Algebra of function composition}
If $f : \mathbbm{X} \mapsto \mathbbm{Y}$ and $g : \mathbbm{Y} \mapsto \mathbbm{Z}$ are injective functions, then $g \circ f : \mathbbm{X} \mapsto \mathbbm{Z}$ is an injective function.

A perfect hash function $\Fun{h}[\mathbbm{A}] : \mathbbm{X} \mapsto \mathbbm{Y}$ is not injective.
However, its restriction,
\begin{equation}
	h(\mathbbm{A})|_{\mathbbm{A}} : \mathbbm{A} \mapsto \mathbbm{Y}
\end{equation}
is injective. It immediately follows that $g \circ h_\mathbbm{A} : \mathbbm{X} \mapsto \mathbbm{Z}$ is injective over the restriction to $\mathbbm{A}$, i.e., it is a perfect hash function over $\mathbbm{A}$ of the type $\mathbbm{X} \mapsto \mathbbm{Z}$.

\begin{theorem}
Let $g : \mathbbm{Y} \mapsto \mathbbm{Z}$ be an injective function, $h_\mathbbm{A}^r : \mathbbm{X} \mapsto \mathbbm{Y}$ be a perfect hash function, and  $|\mathbbm{Z}| = (1 + \alpha) |\mathbbm{Y}|$, $\alpha \geq 0$.
...
\end{theorem}
\begin{proof}
It is given that $h_\mathbbm{A}^r$ has a load factor $r = \frac{|\mathbbm{A}|}{|Y|}$, which means $|\mathbbm{Y}| = \frac{|\mathbbm{A}|}{r}$.
Since $g \circ h_\mathbbm{A}^r$ is an injective function into $\mathbbm{Z}$, the load factor must be $r' = \frac{|\mathbbm{A}|}{|\mathbbm{Z}|}$.
It is also given that $|\mathbbm{Z}| = (1+\alpha) |\mathbbm{Y}|$.
Substituting $|\mathbbm{Z}|$ with $(1+\alpha) |\mathbbm{Y}|$, $|\mathbbm{Y}|$ with $\frac{|\mathbbm{A}|}{r}$, and simplifying yields the result $r' = \frac{r}{1+\alpha}$.
\end{proof}

\begin{theorem}
	Let $g : \mathbbm{W} \mapsto \mathbbm{X}$ be an injective function and let $h_\mathbbm{A}^r : \mathbbm{X} 
	\mapsto \mathbbm{Y}$ be a perfect hash function over $\mathbbm{A}$.
	The composition $h_\mathbbm{A}^r \circ g : \mathbbm{W} \mapsto \mathbbm{Y}$, where $|\mathbbm{A}| = (1 + \alpha) |\mathbbm{W}|$, $\alpha \geq 0$, is a perfect hash function $h_\mathbbm{W}^{r'} : \mathbbm{W} \mapsto \mathbbm{Y}$ where $r' = \frac{r}{1+\alpha}$.
\end{theorem}
\begin{proof}
	It is given that $h_\mathbbm{A}^r$ has a load factor $r = \frac{|\mathbbm{A}|}{|\mathbb{Y}|}}$, which means $|\mathbbm{Y}| = \frac{|\mathbbm{A}|}{r}$.
	It is also given that $|\mathbbm{Z}| = (1+\alpha) |\mathbbm{Y}|$.
	Since $g \circ h_\mathbbm{A}^r$ is an injective function into $\mathbbm{Z}$, we see that the load factor must be $r' = \frac{|\mathbbm{A}|}{|\mathbbm{Z}|}$.
	However, we may substitute $|\mathbbm{Z}|$ with $(1+\alpha) |\mathbbm{Y}|$.
	Substituting $\mathbbm{Y}$ with $\frac{m}{r}$ yields $\frac{|\mathbbm{A}|}{(1 + \alpha) \frac{|\mathbbm{A}|}{r}}$.
	Simplying yields the result $\frac{r}{1+\alpha}$.
\end{proof}

\begin{corollary}
	The composition $h_\mathbbm{A}^r \circ g_\mathbbm{A} : \mathbbm{X} \mapsto \mathbbm{Y}$ is a perfect hash function over some subset $\mathbbm{T}$ of $\mathbbm{W}$ such that $|\mathbbm{T}| \leq |\mathbbm{A}|$.
\end{corollary}

Suppose we have a perfect hash function $h_\mathbbm{A} : \mathbbm{X} \mapsto \mathbbm{Y}$.
Then, all of the perfect hash functions that may be generated by composing them with permutation functions are in the same \emph{equivalence class} with the property that every function in this equivalence contains the same distribution of collisions (and, of course, no collisions between elements of $\mathbbm{A}$).

For instance, suppose we have a permutation function $g : \mathbbm{Y} \mapsto \mathbbm{Y}$ and a perfect hash function $h_\mathbbm{A} : \mathbbm{X} \mapsto \mathbbm{Y}$.
Then, the \emph{orbit} of $g^\infty \circ h_\mathbbm{A}$ generates a subset of this permutation equivalence class where $g^0 = \mathrm{id}_\mathbbm{Y}$.

Therefore, if we generate one perfect hash function over $\mathbbm{A}$, we can immediately construct an entire family of perfect hash functions over $\mathbbm{A}$.
This composition \emph{imbeds} the perfect hash function into a larger family of perfect hash functions.

If we limit our attention to bijective functions in $\mathbbm{Y} \mapsto \mathbbm{Y}$, then there are $|\mathbbm{Y}|!$ bijections in $\mathbbm{Y} \mapsto \mathbbm{Y}$ to choose from, i.e., generating one perfect hash function $g_\mathbbm{A} : \mathbbm{X} \mapsto \mathbbm{Y}$ immediately provides $|\mathbbm{Y}|!$ perfect hash functions of type $\mathbbm{X} \mapsto \mathbbm{Y}$ over $\mathbbm{A}$ by \emph{permuting} the output.

We denote the family of perfect hash functions generated from a given perfect hash function $\Fun{f}[\mathbbm{A}] : \mathbbm{X} \mapsto \mathbbm{Y}$ by $\sigma\left(\Fun{f}[\mathbbm{A}]\right)$.
This is an \emph{equivalence class}, i.e., functions in this set are \emph{isomorphic}.
They have the property that for any pair of functions $g,h \in \sigma f_\mathbbm{A}$, $\{ g(x) \in \mathbbm{Y} | x \in \mathbbm{B}\}$, where $\mathbbm{B} \in 2^{\mathbbm{X}}$

Using the previous construction of generating a family of perfect hash functions over $\mathbbm{A}$ by permuting the output, the ratio of the number of such perfect hash functions to the total number of perfect hash functions is
\begin{equation}
	\frac
	{
		\left(|\mathbbm{Y}| - |\mathbbm{A}|\right)!
	}
	{
		|\mathbbm{Y}|^{|\mathbbm{X}| - |\mathbbm{A}|}
	},
\end{equation}
where $\mathbbm{A} \subseteq \mathbbm{X}$, $|\mathbbm{X}| \geq |\mathbbm{Y}|$, and $|\mathbbm{A}| \leq |\mathbbm{Y}|$.

Suppose we have two perfect hash functions, $g_\mathbbm{A} : \mathbbm{X} \mapsto \mathbbm{Y}$ and $f_\mathbbm{A} : \mathbbm{Y} \mapsto \mathbbm{W}$.
Then, the function $f \circ g : \mathbbm{W} \mapsto \mathbbm{Y}$ is a perfect hash function over $\mathbbm{A}$.

Suppose we have two perfect hash functions, $g_\mathbbm{A} : \mathbbm{X} \mapsto \mathbbm{Y}$ and $f_\mathbbm{B} : \mathbbm{X} \mapsto \mathbbm{Y}$.
Then, we may define the following higher-order operation.

We take the input $x \in \mathbbm{X}$, apply it to both $g_\mathbbm{A}$ and $f_\mathbbm{B}$, yielding respectively $y_1$ and $y_2$ in $\mathbbm{Y}$.
Next, we join the outputs in some way, e.g., the concatenation $y_1 y_2$.
Observe that no other $x \in \mathbbm{A}$ hashes to $y_1$ and no $x \in \mathbbm{B}$ hashes to $y_2$.
Therefore, no other $x \in \mathbbm{A}$ hashes to $y_1 y_2$ and no other $x \in \mathbbm{B}$ hashes to $y_1 y_2$.
...
lower-bounds given by \cref{??} are with respect to \emph{uniformly distributed}
...
\end{theorem}
\begin{proof}
Each iteration of the loop in \cref{alg:ph} has a collision test which is Bernoulli distributed with a probability of success $p(m,r)$, where success denotes no collision occurred. We are interested in the random length $N$ of the bit string when this outcome occurs.
...
In \cref{fig:pmf_fN}, we plot the probability mass function of $N$ conditioned on $m=25$ and several different load factors. We see that the probability mass is peaked and unimodal and tends to nearly zero everywhere except over a concentrated interval around its expected value.
\begin{figure}
\centering
